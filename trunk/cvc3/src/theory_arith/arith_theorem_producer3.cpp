...
////////////////////////////////////////////////////////////////////
// TheoryArith: trusted method for creating ArithTheoremProducer3
////////////////////////////////////////////////////////////////////

ArithProofRules* TheoryArith3::createProofRules3() {
  return new ArithTheoremProducer3(theoryCore()->getTM(), this);
}
...
// ==> x - y = x + (-1) * y
Theorem ArithTheoremProducer3::minusToPlus(const Expr& x, const Expr& y)
{
  Proof pf;
...
// (+ t1 ... tn)/d ==> (+ (t1/d) ... (tn/d))
Theorem ArithTheoremProducer3::canonDividePlus(const Expr& sum, const Expr& d) {
  if(CHECK_PROOFS) {
...
Theorem ArithTheoremProducer3::canonDivideVar(const Expr& e, const Expr& d) {
  if(CHECK_PROOFS) {
    CHECK_SOUND(isRational(d),
                                              ...
                                              const Expr & e)
{
  ...
  DebugAssert ((e.arity() > 1) && (e[0].isRational()),
               "ArithTheoremProducer3::canonMultConstMult: "
...
public:
  bool operator()(const Expr& e1, const Expr& e2) const {
    return ArithTheoremProducer3::greaterthan(e1,e2);
  }
};
...
Expr
ArithTheoremProducer3::canonCombineLikeTerms(const std::vector<Expr> & sumExprs)
{
  Rational constant = 0;
  MonomMap sumHashMap;
  vector<Expr> sumKids;
...
Theorem
ArithTheoremProducer3::canonMultMtermMterm(const Expr& e)
{
  if(CHECK_PROOFS) {
...
Theorem
ArithTheoremProducer3::canonPlus(const Expr& e)
{
  Proof pf;
...
Theorem
ArithTheoremProducer3::canonMult(const Expr& e)
{
  Proof pf;
...
Theorem
ArithTheoremProducer3::canonInvertConst(const Expr& e)
{
  if(CHECK_PROOFS)
...
Theorem
ArithTheoremProducer3::canonInvertLeaf(const Expr& e)
{
  Proof pf;
...
Theorem
ArithTheoremProducer3::canonInvertPow(const Expr& e)
{
...
Theorem
ArithTheoremProducer3::canonInvertMult(const Expr& e)
{
...
Theorem
ArithTheoremProducer3::canonInvert(const Expr& e)
{
  DebugAssert(e.getKind() != PLUS,
...
Theorem
ArithTheoremProducer3::canonDivide(const Expr& e)
{
...
Theorem
ArithTheoremProducer3::canonMultTermConst(const Expr& c, const Expr& t) {
  Proof pf;
  if(CHECK_PROOFS) {
    CHECK_SOUND(isRational(c),
...
Theorem
ArithTheoremProducer3::canonMultTerm1Term2(const Expr& t1, const Expr& t2) {
  // Proof pf;
...
Theorem
ArithTheoremProducer3::canonMultConstConst(const Expr& c1, const Expr& c2) {
  Proof pf;
  if(CHECK_PROOFS) {
...
Theorem
ArithTheoremProducer3::canonMultConstTerm(const Expr& c1,
...
Theorem
ArithTheoremProducer3::canonMultConstSum(const Expr& c1, const Expr& sum) {
  Proof pf;
  std::vector<Expr> sumKids;
...
Theorem
ArithTheoremProducer3::canonPowConst(const Expr& e) {
  if(CHECK_PROOFS) {
    CHECK_SOUND(e.getKind() == POW && e.arity() == 2
		&& e[0].isRational() && e[1].isRational(),
		"ArithTheoremProducer3::canonPowConst("+e.toString()+")");
  }
...
Theorem
ArithTheoremProducer3::canonFlattenSum(const Expr& e) {
  Proof pf;
  std::vector<Expr> sumKids;
  if(CHECK_PROOFS) {
...
Theorem
ArithTheoremProducer3::canonComboLikeTerms(const Expr& e) {
  Proof pf;
  std::vector<Expr> sumKids;
...
Theorem ArithTheoremProducer3::elimPowerConst(const Expr& expr, const Rational& root)
{
  if (CHECK_PROOFS) {
    CHECK_SOUND(expr.isEq() && isPow(expr[0]) &&
                isIntegerConst(expr[0][0]) &&
...
Theorem ArithTheoremProducer3::intEqIrrational(const Expr& expr, const Theorem& isIntx)
{
  if (CHECK_PROOFS) {
    CHECK_SOUND(expr.isEq() && isPow(expr[0]) &&
                expr[1].isRational() &&
    ...
    CHECK_SOUND(isIntPred(isIntx.getExpr()) && isIntx.getExpr()[0] == expr[0][1],
		"ArithTheoremProducer3::intEqIrrational:\n "
...
Theorem ArithTheoremProducer3::constPredicate(const Expr& e) {
  ...
  default:
    if(CHECK_PROOFS) {
      CHECK_SOUND(false,
                  "ArithTheoremProducer3::constPredicate: wrong kind");
    }
    break;
  }
...
Theorem ArithTheoremProducer3::rightMinusLeft(const Expr& e)
{
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND((EQ==kind) ||
                ...
                (GE==kind) ||
                (GT==kind),
                "ArithTheoremProducer3::rightMinusLeft: wrong kind");
  }
...
Theorem ArithTheoremProducer3::plusPredicate(const Expr& x,
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND((EQ==kind) ||
                ...
                (GE==kind) ||
                (GT==kind),
                "ArithTheoremProducer3::plusPredicate: wrong kind");
  }
  Proof pf;
...
Theorem ArithTheoremProducer3::multEqn(const Expr& x,
  ...
  if(CHECK_PROOFS)
    CHECK_SOUND(z.isRational() && z.getRational() != 0,
		"ArithTheoremProducer3::multEqn(): multiplying equation by 0");
...
Theorem ArithTheoremProducer3::multIneqn(const Expr& e, const Expr& z)
{
  int kind = e.getKind();
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND((LT==kind) ||
                ...
                (GE==kind) ||
                (GT==kind),
                "ArithTheoremProducer3::multIneqn: wrong kind");
    CHECK_SOUND(z.isRational() && z.getRational() != 0,
                "ArithTheoremProducer3::multIneqn: "
...
Theorem ArithTheoremProducer3::flipInequality(const Expr& e)
{
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND(isGT(e) || isGE(e),
                "ArithTheoremProducer3::flipInequality: wrong kind: " +
                e.toString());
  }
...
Theorem ArithTheoremProducer3::negatedInequality(const Expr& e)
{
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND(e.isNot(),
                "ArithTheoremProducer3::negatedInequality: wrong kind: " +
                e.toString());
                                         ...
                                         const Theorem& tLTbeta)
{
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND((isLE(expr1) || isLT(expr1)) &&
                ...
                "ArithTheoremProducer3::realShadow: Wrong Kind: " +
                alphaLTt.toString() +  tLTbeta.toString());

    CHECK_SOUND(expr1[1] == expr2[0],
                "ArithTheoremProducer3::realShadow:"
                                           ...
                                           const Theorem& tLEalpha)
{
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND(isLE(expr1) && isLE(expr2),
                "ArithTheoremProducer3::realShadowLTLE: Wrong Kind: " +
                alphaLEt.toString() +  tLEalpha.toString());

    CHECK_SOUND(expr1[1] == expr2[0],
                "ArithTheoremProducer3::realShadowLTLE:"
                " t must be same for both inputs: " +
                alphaLEt.toString() + " , " + tLEalpha.toString());
...
Theorem
ArithTheoremProducer3::finiteInterval(const Theorem& aLEt,
				     const Theorem& tLEac,
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND(isLE(e1) && isLE(e2),
		"ArithTheoremProducer3::finiteInterval:\n e1 = "
		+e1.toString()+"\n e2 = "+e2.toString());
...
Theorem ArithTheoremProducer3::darkGrayShadow2ab(const Theorem& betaLEbx,
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND(isLE(expr1) && isLE(expr2),
                "ArithTheoremProducer3::darkGrayShadow2ab: Wrong Kind: " +
  ...
  if(CHECK_PROOFS) {
    ...
    CHECK_SOUND(isIntPred(isIntAlphaExpr) && isIntAlphaExpr[0] == alpha,
		"ArithTheoremProducer3::darkGrayShadow2ab:\n "
		"wrong integrality constraint:\n alpha = "
		+alpha.toString()+"\n isIntAlpha = "
...
Theorem ArithTheoremProducer3::darkGrayShadow2ba(const Theorem& betaLEbx,
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND(isLE(expr1) && isLE(expr2),
                "ArithTheoremProducer3::darkGrayShadow2ba: Wrong Kind: " +
                betaLEbx.toString() +  axLEalpha.toString());
  }
  ...
  if(CHECK_PROOFS) {
    ...
    // Expressions ax and bx should match on x
    CHECK_SOUND(!isMult(ax) || ax.arity() == 2,
		"ArithTheoremProducer3::darkGrayShadow2ba:\n ax<=alpha: " +
                axLEalpha.toString());
    CHECK_SOUND(!isMult(bx) || (bx.arity() == 2 && bx[1] == x),
		"ArithTheoremProducer3::darkGrayShadow2ba:\n beta<=bx: "
		+betaLEbx.toString()
...
Theorem ArithTheoremProducer3::expandDarkShadow(const Theorem& darkShadow) {
  ...
  if(CHECK_PROOFS){
    CHECK_SOUND(isDarkShadow(theShadow),
		"ArithTheoremProducer3::expandDarkShadow: not DARK_SHADOW: " +
		theShadow.toString());
  }
  Proof pf;
  if(withProof())
...
Theorem ArithTheoremProducer3::expandGrayShadow0(const Theorem& grayShadow) {
  const Expr& theShadow = grayShadow.getExpr();
  if(CHECK_PROOFS) {
    CHECK_SOUND(isGrayShadow(theShadow),
		"ArithTheoremProducer3::expandGrayShadowConst0:"
		" not GRAY_SHADOW: " +
		theShadow.toString());
    CHECK_SOUND(theShadow[2] == theShadow[3],
		"ArithTheoremProducer3::expandGrayShadow0: c1!=c2: " +
		theShadow.toString());
  }
  Proof pf;
...
//       G2 = G(x,e,c+1,c2),
// and    c = floor((c1+c2)/2)
// DEJAN: TRY THE MIDDLE IMMEDIATELY, WE MIGHT GET LUCKY
Theorem ArithTheoremProducer3::splitGrayShadow(const Theorem& gThm) {
  const Expr& theShadow = gThm.getExpr();
  if(CHECK_PROOFS) {
    CHECK_SOUND(isGrayShadow(theShadow),
		"ArithTheoremProducer3::expandGrayShadowConst: not a shadow" +
		theShadow.toString());
  }
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND(c1.isInteger() && c2.isInteger() && c1 < c2,
		"ArithTheoremProducer3::expandGrayShadow: " +
		theShadow.toString());
  }
...
Theorem ArithTheoremProducer3::expandGrayShadow(const Theorem& gThm) {
  const Expr& theShadow = gThm.getExpr();
  if(CHECK_PROOFS) {
    CHECK_SOUND(isGrayShadow(theShadow),
		"ArithTheoremProducer3::expandGrayShadowConst: not a shadow" +
...
Theorem
ArithTheoremProducer3::expandGrayShadowConst(const Theorem& gThm) {
  const Expr& theShadow = gThm.getExpr();
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND(!isMult(ax) || ax[0].isRational(),
		"ArithTheoremProducer3::expandGrayShadowConst: "
  ...
  if(CHECK_PROOFS) {
    ...
    CHECK_SOUND(bExpr.isRational() && bExpr.getRational().isInteger(),
		"ArithTheoremProducer3::expandGrayShadowConst: b not integer: "
		+theShadow.toString());
  }
...
Theorem
ArithTheoremProducer3::grayShadowConst(const Theorem& gThm) {
  const Expr& g = gThm.getExpr();
  bool checkProofs(CHECK_PROOFS);
  if(checkProofs) {
    CHECK_SOUND(isGrayShadow(g), "ArithTheoremProducer3::grayShadowConst("
		+g.toString()+")");
  }
  ...
  if(checkProofs) {
    CHECK_SOUND(e.isRational() && e.getRational().isInteger(),
		"ArithTheoremProducer3::grayShadowConst("+g.toString()+")");
...
Rational ArithTheoremProducer3::constRHSGrayShadow(const Rational& c,
  ...
  DebugAssert(c.isInteger() &&
	      ...
	      a.isInteger() &&
	      b != 0,
	      "ArithTheoremProducer3::constRHSGrayShadow: a, b, c must be ints");
  if (b > 0)
    return mod(c+b, a);
  else
    return mod(a-(c+b), a);
...
Theorem ArithTheoremProducer3::lessThanToLE(const Theorem& less,
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND(isLT(ineq),
		"ArithTheoremProducer3::LTtoLE: ineq must be <");
    // Integrality check
    CHECK_SOUND(isIntPred(isIntLHSexpr)	&& isIntLHSexpr[0] == ineq[0],
		"ArithTheoremProducer3::lessThanToLE: bad integrality check:\n"
		" ineq = "+ineq.toString()+"\n isIntLHS = "
 ...
 */
Theorem
ArithTheoremProducer3::intVarEqnConst(const Expr& eqn,
				     const Theorem& isIntx) {
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND((isMult(right) && right[0].isRational())
                ...
                "ArithTheoremProducer3::intVarEqnConst: "
		"rhs has a wrong format: " + right.toString());
    CHECK_SOUND(left.isRational() && 0 == left.getRational(),
                "ArithTheoremProducer3:intVarEqnConst:left is not a zero: " +
                left.toString());
  }
  ...
  if(CHECK_PROOFS) {
    ...
    CHECK_SOUND(a!=0, "ArithTheoremProducer3:intVarEqnConst: eqn = "
		+eqn.toString());
  }
  const Assumptions& assump(isIntx.getAssumptionsRef());
...
Expr
ArithTheoremProducer3::create_t(const Expr& eqn) {
  ...
  DebugAssert(m > 0, "ArithTheoremProducer3::create_t: m = "+m.toString());
  vector<Expr> kids;
  if(isPlus(eqn[1]))
...
Expr
ArithTheoremProducer3::create_t2(const Expr& lhs, const Expr& rhs,
				const Expr& sigma) {
  Rational m = lhs[0].getRational()+1;
  DebugAssert(m > 0, "ArithTheoremProducer3::create_t2: m = "+m.toString());
  vector<Expr> kids;
  if(isPlus(rhs))
...
Expr
ArithTheoremProducer3::create_t3(const Expr& lhs, const Expr& rhs,
				const Expr& sigma) {
  const Rational& a = lhs[0].getRational();
  Rational m = a+1;
  DebugAssert(m > 0, "ArithTheoremProducer3::create_t3: m = "+m.toString());
  vector<Expr> kids;
  if(isPlus(rhs))
...
Rational
ArithTheoremProducer3::modEq(const Rational& i, const Rational& m) {
  DebugAssert(m > 0, "ArithTheoremProducer3::modEq: m = "+m.toString());
  Rational half(1,2);
...
Rational
ArithTheoremProducer3::f(const Rational& i, const Rational& m) {
  DebugAssert(m > 0, "ArithTheoremProducer3::f: m = "+m.toString());
  Rational half(1,2);
  return (floor(i/m + half)+modEq(i,m));
}

void
ArithTheoremProducer3::sumModM(vector<Expr>& summands, const Expr& sum,
                              const Rational& m, const Rational& divisor) {
  DebugAssert(divisor != 0, "ArithTheoremProducer3::sumModM: divisor = "
	      +divisor.toString());
  Expr::iterator i = sum.begin();
...
Expr
ArithTheoremProducer3::monomialModM(const Expr& i,
                                   const Rational& m, const Rational& divisor)
{
  DebugAssert(divisor != 0, "ArithTheoremProducer3::monomialModM: divisor = "
	      +divisor.toString());
  Expr res;
  if(isMult(i)) {
  ...
  DebugAssert(!res.isNull(), "ArithTheoremProducer3::monomialModM()");
...
void
ArithTheoremProducer3::sumMulF(vector<Expr>& summands, const Expr& sum,
                              const Rational& m, const Rational& divisor) {
  DebugAssert(divisor != 0, "ArithTheoremProducer3::sumMulF: divisor = "
	      +divisor.toString());
  Expr::iterator i = sum.begin();
  DebugAssert(i != sum.end(), CLASS_NAME "::sumModM");
  Rational C = i->getRational();
...
Expr
ArithTheoremProducer3::monomialMulF(const Expr& i,
                                   const Rational& m, const Rational& divisor)
{
  DebugAssert(divisor != 0, "ArithTheoremProducer3::monomialMulF: divisor = "
	      +divisor.toString());
...
Expr
ArithTheoremProducer3::substitute(const Expr& term, ExprMap<Expr>& eMap)
{
...
bool ArithTheoremProducer3::greaterthan(const Expr & l, const Expr & r)
{
  //    DebugAssert(l != r, "");
  if (l==r) return false;
...
Theorem
ArithTheoremProducer3::eqElimIntRule(const Theorem& eqn, const Theorem& isIntx,
  ...
  if(CHECK_PROOFS)
    CHECK_SOUND(eqn.isRewrite(),
                "ArithTheoremProducer3::eqElimInt: input must be an equation" +
                eqn.toString());
  ...
  if(CHECK_PROOFS) {
    ...
    CHECK_SOUND(isIntPred(isIntxe) && isIntxe[0] == x,
		"ArithTheoremProducer3::eqElimInt\n eqn = "
    ...
    if(!isPlus(rhs)) {
      ...
      CHECK_SOUND(isIntVars.size() + 1 == (size_t)rhs.arity(),
		  "ArithTheoremProducer3::eqElimInt: rhs = "+rhs.toString());
...
Theorem
ArithTheoremProducer3::isIntConst(const Expr& e) {
  Proof pf;

  if(CHECK_PROOFS) {
    CHECK_SOUND(isIntPred(e) && e[0].isRational(),
		"ArithTheoremProducer3::isIntConst(e = "
		+e.toString()+")");
  }
  if(withProof())
...
Theorem
ArithTheoremProducer3::equalLeaves1(const Theorem& thm)
{
  Proof pf;
...
Theorem
ArithTheoremProducer3::equalLeaves2(const Theorem& thm)
{
  Proof pf;
  const Expr& e = thm.getRHS();
...
Theorem
ArithTheoremProducer3::equalLeaves3(const Theorem& thm)
{
  Proof pf;
  const Expr& e = thm.getRHS();
...
Theorem
ArithTheoremProducer3::equalLeaves4(const Theorem& thm)
{
  Proof pf;
  const Expr& e = thm.getRHS();
...
Theorem
ArithTheoremProducer3::diseqToIneq(const Theorem& diseq) {
  Proof pf;
  ...
  if(CHECK_PROOFS) {
    CHECK_SOUND(e.isNot() && e[0].isEq(),
		"ArithTheoremProducer3::diseqToIneq: expected disequality:\n"
		" e = "+e.toString());
  }
...
Theorem ArithTheoremProducer3::oneElimination(const Expr& e) {
...
Theorem ArithTheoremProducer3::clashingBounds(const Theorem& lowerBound, const Theorem& upperBound) {
...
Theorem ArithTheoremProducer3::addInequalities(const Theorem& thm1, const Theorem& thm2) {
...
Theorem ArithTheoremProducer3::implyWeakerInequality(const Expr& expr1, const Expr& expr2) {
...
Theorem ArithTheoremProducer3::implyNegatedInequality(const Expr& expr1, const Expr& expr2) {
...
Theorem ArithTheoremProducer3::trustedRewrite(const Expr& expr1, const Expr& expr2) {
...
Theorem ArithTheoremProducer3::integerSplit(const Expr& intVar, const Rational& intPoint) {
...
Theorem ArithTheoremProducer3::rafineStrictInteger(const Theorem& isIntConstrThm, const Expr& constr) {
...
Theorem ArithTheoremProducer3::splitGrayShadowSmall(const Theorem& gThm) {
	DebugAssert(false, "Not implemented!!!");
	return Theorem();
}

Theorem ArithTheoremProducer3::implyWeakerInequalityDiffLogic(const std::vector<Theorem>& antecedentThms, const Expr& implied) {
	DebugAssert(false, "Not implemented!!!");
	return Theorem();
}

Theorem ArithTheoremProducer3::implyNegatedInequalityDiffLogic(const std::vector<Theorem>& antecedentThms, const Expr& implied) {
	DebugAssert(false, "Not implemented!!!");
	return Theorem();
}
...
Theorem ArithTheoremProducer3::addInequalities(const vector<Theorem>& thms) {
	DebugAssert(false, "Not implemented!!!");
	return Theorem();
}
...
Theorem ArithTheoremProducer3::simpleIneqInt(const Expr& ineq, const Theorem& isIntRHS)
{
	DebugAssert(false, "Not implemented!!!");
	return Theorem();
}
...
// deduce bot
Theorem ArithTheoremProducer3::intEqualityRationalConstant(const Theorem& isIntConstrThm, const Expr& constr) {
	DebugAssert(false, "Not implemented!!!");
	return Theorem();
}

Theorem ArithTheoremProducer3::cycleConflict(const vector<Theorem>& inequalitites) {
	DebugAssert(false, "Not implemented!!!");
	return Theorem();
}

Theorem ArithTheoremProducer3::implyEqualities(const vector<Theorem>& inequalitites) {
	DebugAssert(false, "Not implemented!!!");
	return Theorem();
}
